...
class TermInfoQueriesTest(unittest.TestCase):
    ...
    def test_label_stray_backslash_cleaned(self):
        # Upstream Cypher / string-literal escaping can leave a stray backslash
        # before an apostrophe in a label (e.g. "y5β\\'2a"). It must be
        # stripped at ingestion so Name/Meta/links never carry it -- a backslash
        # surviving into a downstream request target triggers a Tomcat
        # "Invalid character found in the request target" 400.
        terminfo_json = r'''
        {"term": {"core": {"iri": "http://n2o.neo/individual/VFB_jrchk0e3",
          "symbol": "", "types": ["Entity", "Individual", "Neuron"],
